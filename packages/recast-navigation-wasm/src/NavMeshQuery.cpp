...
dtStatus NavMeshQuery::queryPolygons(const float *center, const float *halfExtents, const dtQueryFilter *filter, dtPolyRef *polys, IntRef *polyCount, const int maxPolys)
{
    return m_navQuery->queryPolygons(center, halfExtents, filter, polys, &polyCount->value, maxPolys);
}
